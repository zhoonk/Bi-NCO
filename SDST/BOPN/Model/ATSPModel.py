...
class Cross_Encoder(nn.Module):
    def __init__(self, **model_params):
        ...
        node_cnt = model_params['node_cnt']
        embedding_dim = model_params['embedding_dim']

        self.node_idx_projection = nn.Linear(1, embedding_dim)
        self.edge_mtrx_projection1 = nn.Linear(1, embedding_dim)
        self.edge_mtrx_projection2 = nn.Linear(1, embedding_dim)
    
    def compute_normalized_matrices(self, dist_matrices):
        std = torch.std(dist_matrices.reshape(dist_matrices.shape[0], -1), dim=-1)
        a = torch.exp(-torch.square(dist_matrices / std.reshape(-1, 1, 1))).float()
        # D - (diagonal node degree matrix)^(-1/2)
        d = torch.diagonal_scatter(torch.zeros(a.shape, device=a.device),
                                torch.pow(torch.sum(a, dim=-1), -0.5), dim1=1, dim2=2)
        # normalized A: D'^(-1/2) @ A' @ D'^(-1/2)
        norm_matrices = torch.einsum("xab,xbc,xcd->xad", d, a, d)
    ...
    def forward(self, data):
        ...
        edge_emb1 = self.edge_mtrx_projection1(data.float().unsqueeze(-1))
        edge_emb2 = self.edge_mtrx_projection2(data.float().transpose(2,1).unsqueeze(-1))

        for layer in self.layers:
            out1, out2 = layer(out1, out2, edge_emb1, edge_emb2)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, row_emb, col_emb, edge_emb1, edge_emb2):
        # row_emb.shape: (batch, row_cnt, embedding)
        # col_emb.shape: (batch, col_cnt, embedding)
        # cost_mat.shape: (batch, row_cnt, col_cnt)
        row_emb_out = self.row_encoding_block(row_emb, col_emb, edge_emb1)
        col_emb_out = self.col_encoding_block(col_emb, row_emb, edge_emb2)
